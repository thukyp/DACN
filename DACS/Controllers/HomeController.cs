...
namespace DACS.Controllers
{
    public class HomeController : Controller
    {
        ...
        // ... (Các phần code Chatbot giữ nguyên không thay đổi) ...
        [HttpPost]
        public async Task<JsonResult> Ask([FromForm] string message)
        {
            ...
            using (var httpClient = new HttpClient())
            {
                ...
                // Gửi sang API Python
                // Lưu ý: Cần đảm bảo API Python đang chạy
                try
                {
                ...
                {
                    traLoi = "Không thể kết nối tới Server Chatbot.";
                }
            }
        ...
        public async Task<IActionResult> UploadImage(IFormFile file)
        {
            ...
            var uploadPath = Path.Combine(_webHostEnvironment.WebRootPath, "images/Uploads", file.FileName);
            ...
            // Cập nhật lại đường dẫn tuyệt đối cho phù hợp với môi trường của bạn
            string pythonExe = @"C:\Users\hoa23\AppData\Local\Programs\Python\Python312\python.exe";
            string scriptPath = Path.Combine(Directory.GetCurrentDirectory(), @"DACS\Services\compare_images.py"); // Nên dùng Path.Combine
            string datasetFolder = Path.Combine(_webHostEnvironment.WebRootPath, @"images\Products");

            // ... (Phần gọi Process Python giữ nguyên) ...
            // Lưu ý: Process.Start có thể cần try-catch để xử lý trường hợp không tìm thấy python

            return Ok(new { reply = "Đang xử lý ảnh..." }); // Tạm thời trả về, bạn giữ logic cũ
        }
